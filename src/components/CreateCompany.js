...
class CreateCompany extends Component {
    ...
    saveOrUpdateCompany = (e) => {
        e.preventDefault();
        let company = {comName: this.state.comName,comAddress: this.state.comAddress, comEmail: this.state.comEmail, comDescription: this.state.comDescription};
        console.log('company => ' + JSON.stringify(company));

        // step 5
        if(this.state.id === '_add'){
            CompanySevice.createCompany(company).then(res =>{
                this.props.history.push('/companys_list');
            });
        }else{
